...
from courses.models import (
    Category, Course, Lesson, Enrollment, Quiz, Question, Answer
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Starting demo data creation...')
        )
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'Created {len(instructors)} instructors and {len(students)} students')
                )

                # Create courses
                courses = self._create_courses(courses_count, categories, instructors)
                self.stdout.write(
                    self.style.SUCCESS(f'Created {len(courses)} courses')
                )
            ...
            self.stdout.write(
                self.style.WARNING('Run `python manage.py collectstatic` to serve thumbnails')
            )
    ...
    def _create_courses(self, count, categories, instructors):
        """Create demo courses"""
        course_data = [
            ('Complete Python Bootcamp', 'Learn Python from scratch to advanced', 'beginner', '12 weeks', 89.99),
            ('Advanced Web Development', 'Build modern web applications', 'intermediate', '10 weeks', 129.99),
            ('Data Science Masterclass', 'Machine learning and data analysis', 'intermediate', '16 weeks', 149.99),
            ('Digital Marketing Pro', 'SEO, PPC, and social media marketing', 'beginner', '8 weeks', 79.99),
            ('React Native Mobile Apps', 'Build cross-platform mobile apps', 'intermediate', '6 weeks', 99.99),
            ('AWS Cloud Practitioner', 'Amazon Web Services fundamentals', 'beginner', '4 weeks', 59.99),
            ('UI/UX Design Essentials', 'Figma and design principles', 'beginner', '8 weeks', 69.99),
            ('Cybersecurity Fundamentals', 'Network security basics', 'beginner', '6 weeks', 89.99),
            ('DevOps Engineering', 'CI/CD pipelines and automation', 'intermediate', '12 weeks', 119.99),
            ('Machine Learning Deep Dive', 'Neural networks and deep learning', 'advanced', '20 weeks', 199.99),
        ]
    ...
    def _create_quizzes_fixed(self, courses):
        ...
        for course in courses:
            ...
            for i in range(num_quizzes):
                ...
                quiz.save(update_fields=['total_marks'])  # Direct update without triggering save() logic
